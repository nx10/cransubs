...
import type { CranQueue, CranQueueInfo, Submission, SnapshotContainer } from './types.js';
...
export const CRAN_QUEUES: { [qid: string]: CranQueueInfo } = {
	newbies: {
		...
		longdescription:
			'This is either a new package or a package to be unarchived. ' +
			'Such packages are queued for manual inspection.',
		reference: 'https://journal.r-project.org/archive/2018-1/cran.pdf',
		reference_label:
			'Learn more: Hornik, K., Ligges, U., & Zeileis, A. (2018). Changes on CRAN. 2018-07-01 to 2018-12-31. The R Journal, 10(2), 1-3.'
	},
	inspect: {
		order: 1,
		name: 'Inspect',
		...
		reference: 'https://journal.r-project.org/archive/2018-1/cran.pdf',
		reference_label:
			'Hornik, K., Ligges, U., & Zeileis, A. (2018). Changes on CRAN. 2018-07-01 to 2018-12-31. The R Journal, 10(2), 1-3.'
	},
	pretest: {
		order: 2,
		name: 'Pretest',
	...
	publish: {
		...
		longdescription:
			'"Publish the package, if the package is already well established on CRAN, passes ' +
...
export async function getSnap(): Promise<SnapshotContainer> {
	const response = await fetch('https://cransubs.fly.dev/snap');
	return (await response.json()) as SnapshotContainer;
}
...
export class SubmissionRepo {
	...
	constructor(snapContainer?: SnapshotContainer, subRepo?: SubmissionRepo) {
		if (snapContainer) {
			...
			const dict: { [folder: string]: Submission[] } = {};
	...
	private sort_queues() {
		...
		for (const qu of this.queues) {
			qu.queue.sort((sub_a, sub_b) => {
				return compareAsc(parseISO(sub_a.file_time), parseISO(sub_b.file_time));
			});
		}
	}
	...
	public find_pkg(name: string, file_time?: string): [Submission, CranQueue] | undefined {
		if (file_time === undefined) {
			const pot_subs: [Submission, CranQueue][] = [];
			for (const qu of this.queues) {
			...
			return pot_subs
				.sort((sub_a, sub_b) => {
					return compareAsc(parseISO(sub_b[0].file_time), parseISO(sub_a[0].file_time));
				})
				.at(0);
		} else {
			for (const qu of this.queues) {
	...
	public filter(str: string): SubmissionRepo {
		...
		re.queues = this.queues
			.map((qu) => ({
				info: qu.info,
				queue: qu.queue
					.map((sub, idx) => ({
						...sub,
						original_position: sub.original_position ?? idx + 1
					}))
